...
<div class="block_with_links_under_form">
    ...
    <a class="links_under_form" href="/<?php echo $table_name; ?>/add">Добавить ИБП</a>
</div>
<hr>
    ...
    <table class="table_under_form">
        ...
        <?php foreach ($current_table as $row): ?>
            <tr>
                ...
                <?php
                echo '<td>';
                if ($row->battery->bat_voltage && $row->battery->bat_capacity)
                {
                    echo $row->battery->bat_voltage.'V - '.$row->battery->bat_capacity.'A/h';
                }
                echo '</td>';
                ?>
                <?='<td>'.$row->bat_count.'</td>'?>
