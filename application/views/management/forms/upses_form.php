...
<div class="editable_line_block">
    ...
    <table class="table_above_form">
        ...
        <tr>
            ...
            <?='<td>'.$editable_line->battery->bat_voltage.'V - '.
                      $editable_line->battery->bat_capacity.'A/h</td>'?>
            <?='<td>'.$editable_line->bat_count.'</td>'?>
...
<div class="block_inside_form">
    <div class="form_block_pair">
        ...
        <div class="label_and_select">
            <label class="form_label"><?=$field_bat_id?>:</label>
            <select class="input_text" name="bat_id">
                <option value="">Выберите тип АКБ</option>
                <?php foreach ($select_battypes as $sel_battype): ?>
                <option value="<?=$sel_battype->bat_id?>"
                <?php
                if (isset($res_status) && $res_status == 'fail' && isset($bat_id))
                    if ($sel_battype->bat_id == $bat_id) echo ' selected';
                ?>
                ><?=$sel_battype->bat_voltage.'V - '.$sel_battype->bat_capacity.'A/h'?></option>
            <?php endforeach; ?>
            </select>
        </div>

        <div class="label_and_select">
            <label class="form_label"><?=$field_bat_count?>:</label>
            <select class="input_text" name="bat_count">
                <option value="">Выберите кол-во АКБ</option>
                <?php foreach ($select_bat_count as $sel_bat_count): ?>
                <option value="<?=$sel_bat_count?>"
                <?php
                if (isset($res_status) && $res_status == 'fail' && isset($bat_count))
                    if ($sel_bat_count == $bat_count) echo ' selected';
                ?>
                ><?=$sel_bat_count?></option>
            <?php endforeach; ?>
            </select>
        </div>
