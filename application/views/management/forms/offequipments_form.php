...
<div class="editable_line_block">
    ...
    <table class="table_above_form">
        ...
        <tr>
            ...
            <?='<td>'.$editable_line->format.'</td>'?>
            <td>
                <?php
                    foreach ($select_print_tech as $pt_key => $pt_val)
                    {
                        if ($editable_line->print_tech == $pt_key) echo $pt_val;
                    }
                ?>
            </td>
            <td>
                <?php
                    ...
                    {
                        if ($editable_line->print_color == $pc_key) echo $pc_val;
                    }
                ?>
            </td>
...
<div class="block_inside_form">
    <div class="form_block_pair">
        ...
        <div class="label_and_select">
            <label class="form_label"><?=$field_format?>:</label>
            <select class="input_text" name="format">
                <option value="">Выберите формат бумаги</option>
                <?php foreach ($select_format as $sel_format): ?>
                <option value="<?=$sel_format?>"
                <?php
                if (isset($res_status) && $res_status == 'fail' && isset($format))
                    if ($sel_format == $format) echo ' selected';
                ?>
                ><?=$sel_format?></option>
            <?php endforeach; ?>
            </select>
        ...
        <div class="label_and_select">
            <label class="form_label"><?=$field_print_tech?>:</label>
            <select class="input_text" name="print_tech">
                <option value="">Выберите технологию печати</option>
                <?php foreach ($select_print_tech as $pt_key => $pt_val): ?>
                <option value="<?=$pt_key?>"
                <?php
                if (isset($res_status) && $res_status == 'fail' && isset($print_tech))
                    if ($pt_key == $print_tech) echo ' selected';
                ?>
                ><?=$pt_val?></option>
            <?php endforeach; ?>
            </select>
        ...
        <div class="label_and_select">
            <label class="form_label"><?=$field_print_color?>:</label>
            <select class="input_text" name="print_color">
                <option value="">Выберите цветность печати</option>
                <?php foreach ($select_print_color as $pc_key => $pc_val): ?>
                <option value="<?=$pc_key?>"
                <?php
                if (isset($res_status) && $res_status == 'fail' && isset($print_color))
                    if ($pc_key == $print_color) echo ' selected';
                ?>
                ><?=$pc_val?></option>
            <?php endforeach; ?>
            </select>
