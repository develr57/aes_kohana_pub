...
class Model_Offequipment extends ORM {

    protected $_table_name = 'offequipments';
    protected $_primary_key = 'oe_id';
    ...
    public $table_name          = 'offequipments';
    public $id_column           = 'oe_id';

    public $select_format       = array(
        'A3', 'A4', 'A5',
    );

    public $select_print_tech   = array(
        'L'     =>  'Лазерная',
        'I'     =>  'Чернильная',
        'M'     =>  'Матричная',
    );

    public $select_print_color  = array(
        'B'     =>  'нет',
        'C'     =>  'есть',
    );
    ...
    public function getDefaultVars()
    {
        return array(
            ...
            'field_model'       => 'Модель',
            'field_format'      => 'Формат',
            'field_print_tech'  => 'Технология печати',
            'field_print_color' => 'Цветная печать',
            ...
            'select_print_tech' => $this->select_print_tech,
            'select_print_color'=> $this->select_print_color,
        );
    }
    ...
    {
        $query = ORM::factory('Offequipment')
    ...
    public function addOffequipment($to_add)
    {
        extract($to_add);
        ...
        if ($this->IDCheck($check_IDs))
        {
                ...
                )
            {
                ...
                } else
                {
                    ...
                    {
                        $oper_add = ORM::factory('Offequipment');
                        $oper_add->devtype_id   =   $devtype_id;
                        $oper_add->vendor_id    =   $vendor_id;
                        $oper_add->model        =   $model;
                        $oper_add->format       =   $format;
                        $oper_add->print_tech   =   $print_tech;
                        $oper_add->print_color  =   $print_color;
    ...
    public function editOffequipment($to_edit)
    {
        extract($to_edit);
        ...
        if ($this->IDCheck($check_IDs))
        {
            ...
            {    
                ...
                {
                    $oper_edit = ORM::factory('Offequipment', $id);
                    $oper_edit->devtype_id   =   $devtype_id;
                    $oper_edit->vendor_id    =   $vendor_id;
                    $oper_edit->model        =   $model;
                    $oper_edit->format       =   $format;
                    $oper_edit->print_tech   =   $print_tech;
                    $oper_edit->print_color  =   $print_color;
    ...
    } //End editOffeqipment function

} // End Offequipment Model