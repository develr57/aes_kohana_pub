...
class Controller_Vendors extends Controller_Base {
    ...
    {
        View::set_global('title', 'Таблица вендоров');
        View::bind_global('result', $result);
        $this->template->content = View::factory('management/system_content', array(
            'current_table' =>  ORM::factory('Vendor')->find_all(),
            'def_vars'      =>  ORM::factory('Vendor')->getDefaultVars(),
        ));
    ...
    public function action_add()
    {
        if ($this->auth->logged_in('admin'))
        {
            if (isset($_POST['text_value']))
            {
                ...
                $result = ORM::factory('Vendor')->addVendor($text_value);
            }
            View::set_global('title', 'Добавление вендора');
            View::set_global('oper_type', 'add');
            View::set_global('def_vars', ORM::factory('Vendor')->getDefaultVars());
            View::bind_global('result', $result);
    ...
    public function action_edit()
    {
        if ($this->auth->logged_in('admin'))
        {
            ...
            if ($id)
            {
                $edition_row = ORM::factory('Vendor', $id);
                if ($edition_row->loaded())
                {
                    $result = array(
                        'id'            => $id,
                        'text_value'    => $edition_row->vendor_name,
                        'res_status'    => 'fail',
                    );
            ...
            if (isset($_POST['text_value']))
            {
                ...
                $result = ORM::factory('Vendor')->editVendor($to_edit);
            ...
            View::set_global('title', 'Редактирование вендора');
    ...
    public function action_delete()
    {
        if ($this->auth->logged_in('admin'))
        {
            ...
            if ($id)
            {
                $delete_row = ORM::factory('Vendor', $id);
