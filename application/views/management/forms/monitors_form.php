...
<div class="block_inside_form">
    <div class="form_block_pair">
        ...
        <div class="label_and_select">
            <label class="form_label"><?=$field_diagonal?>:</label>
            <select class="input_text" name="diagonal" required>
                <option value="">Выберите диагональ</option>
                <?php foreach ($select_diagonals as $sel_diagonal): ?>
                <option value="<?=$sel_diagonal?>"
                <?php
                    if (isset($res_status) && $res_status == 'fail' && isset($diagonal))
                        if ($sel_diagonal == $diagonal) echo ' selected';
                ?>
                ><?=$sel_diagonal?></option>
            <?php endforeach; ?>
            </select>
