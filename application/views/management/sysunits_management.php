...
<div class="block_with_links_under_form">
    ...
    <a class="links_under_form" href="/<?php echo $table_name; ?>/add">Добавить системный блок</a>
</div>
<hr>
    ...
    <table class="table_under_form">
        ...
        <?php foreach ($current_table as $row): ?>
            <tr>
                ...
                ?>
                <?='<td>'.$row->note.'</td>'?>
                <?='<td><a href="/'.$table_name.'/configuration/'.$row->$id_column.'">Конфигурация</a></td>'?>
                <?='<td><a href="/'.$table_name.'/edit/'.$row->$id_column.'">Изменить</a></td>'?>
                <?='<td><a href="/'.$table_name.'/delete/'.$row->$id_column.'">Удалить</a></td>'?>
                <?='<td><a href="/'.$table_name.'/delete_with_all_units/'.$row->$id_column.'">Удалить с комплектующими</a></td>'?>
            </tr>
        <?php endforeach; ?>
    </table>
</div>