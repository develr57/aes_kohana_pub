...
class Controller_Monitors extends Controller_Base {
    ...
    {
        View::set_global('title', 'Таблица мониторов');
        $this->template->content = View::factory('management/monitors_management', array(
            'current_table' =>  ORM::factory('Monitor')->find_all(),
            'def_vars'      =>  ORM::factory('Monitor')->getDefaultVars(),
        ));
    ...
    public function action_add()
    {
        if ($this->auth->logged_in('admin'))
        {
            ...
            {    
                ...
                $result = ORM::factory('Monitor')->addMonitor($to_add);
            }

            View::set_global('title', 'Добавление монитора');
            View::set_global('oper_type', 'add');
            View::set_global('def_vars', ORM::factory('Monitor')->getDefaultVars());
            View::bind_global('result', $result);
            $this->template->set_filename('templates/forms_common');

            $devgroup = ORM::factory('Devgroup')
                ->where('devgroup_name', '=', 'Монитор')
                ->find();
            
            $this->template->content = View::factory('management/forms/monitors_form', array(
    ...
    public function action_edit()
    {
        if ($this->auth->logged_in('admin'))
        {
            ...
            if ($id)
            {
                $editable_line = ORM::factory('Monitor', $id);
                if ($editable_line->loaded())
                {
                    $result = array(
                        ...
                        'diagonal'      => $editable_line->diagonal,
            ...
            {    
                ...
                $result = ORM::factory('Monitor')->editMonitor($to_edit);
            ...
            View::set_global('title', 'Редактирование монитора');
    ...
    public function action_delete()
    {
        if ($this->auth->logged_in('admin'))
        {
            ...
            if ($id)
            {
                $delete_line = ORM::factory('Monitor', $id);
    ...
    public function action_choice()
    {
        if ($this->auth->logged_in('admin'))
        {
            ...
            View::set_global('title', 'Выбор монитора');
            View::set_global('def_vars', ORM::factory('Monitor')->getDefaultVars());
            ...
            $this->template->content = View::factory('main_logic/choice_monitor', array(
